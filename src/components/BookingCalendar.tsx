...
const BookingCalendar: React.FC<BookingCalendarProps> = ({
  ...
  const isLongWorkshopSelected = (day: string) => {
    ...
    const selectedWorkshopsForDay = getAfternoonWorkshops(day).filter(
      (w: Workshop) => isAfternoonWorkshopSelected(day, w.id)
    );

    return selectedWorkshopsForDay.some((workshop: Workshop) =>
      workshop.atelier.includes("Le RH est il un psy ?")
    );
  };
    ...
    <div className="bg-white rounded-xl shadow-lg p-6 mb-8 border-2 border-primary overflow-hidden">
      ...
      <div className="space-y-10">
        {Object.keys(workshopsByDay).map((day) => {
          ...
          const selectedAfternoonCount = afternoonWorkshops.filter(
            (w: Workshop) => isAfternoonWorkshopSelected(day, w.id)
          ).length;
          ...
          return (
            ...
            >
              ...
              {isExpanded ? (
                    ...
                    <div>
                      <h4 className="font-bold text-primary mb-3 flex items-center justify-between">
                        <div className="flex items-center">
                          ...
                          Activités de l'après-midi (choisissez-en{" "}
                          {requiredCount})
                          {longWorkshopSelected &&
                            " - Atelier long sélectionné"}
                        </div>
                        <span
                      ...
                      <table className="w-full border-collapse min-w-full">
                        ...
                        <tbody>
                          {afternoonWorkshops.map((workshop: Workshop) => {
                            ...
                            const isLongWorkshop = workshop.atelier.includes(
                              "Le RH est il un psy ?"
                            );
                ...
                <div className="p-4 text-center text-gray-500 py-10 bg-[url('https://www.transparenttextures.com/patterns/clean-textile.png')]">
